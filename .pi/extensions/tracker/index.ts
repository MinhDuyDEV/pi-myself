...
import { type TrackerParams, trackerSchema } from "./params.js";
...
export default function trackerExtension(pi: ExtensionAPI): void {
	pi.registerTool({
		...
		label: "Tracker",
		description:
			"Read or update the repo's issue tracker the way Matt Pocock's skills expect (to-spec, to-tickets, wayfinder, triage). Local markdown (.scratch/) ops: list (filter by status/category/type), frontier, show, create-spec, create-ticket, create-map, claim, resolve, out-of-scope, tick, status, block, comment, edit (title/body), note (append a map section line). GitHub Issues ops (gh-* prefix, need the gh CLI + auth): gh-list (label filter via status, mapped through triage-labels.md), gh-frontier (optionally scoped to a parent), gh-triage (attention queue), gh-show (full comments), gh-create-spec, gh-create-ticket (native sub-issue + dependency edges mirrored by Part of / Blocked by lines; wayfinder type → wayfinder:<type> label), gh-create-map, gh-claim (assign @me), gh-resolve (answer comment + close + gist into the map), gh-out-of-scope, gh-comment, gh-status (swaps only the state-role or category-role family), gh-block (adds and removes native edges), gh-tick, gh-edit, gh-note.",
		...
		async execute(_toolCallId: string, params: TrackerParams, _signal: AbortSignal | undefined, _onUpdate: unknown) {
