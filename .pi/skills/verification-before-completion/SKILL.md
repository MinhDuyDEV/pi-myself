...
A green check proves nothing if the diff lowered the bar to reach it. Before any completion claim, read `git diff` (staged and unstaged, against the branch point) for these five moves:
...
Tightening the bar is silent; loosening it is loud. Every hit is either reverted or named in the final message with its reason. A hit the task did not call for makes the work `partial`, not done.
