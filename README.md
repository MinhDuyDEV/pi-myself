...
| Task roles | `explore` / `scout` / `general` / `reviewer` overrides for `pi-task` plus harness-authored `designer` (design-it-twice parallel candidates) and `researcher` (writes the cited research artifact) — delegation contracts and a 1-writer + 1-reviewer WIP cap |
