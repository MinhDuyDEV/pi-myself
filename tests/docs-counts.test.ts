...
const toNumber = (token: string): number => (/^\d+$/.test(token) ? Number(token) : (WORDS[token.toLowerCase()] ?? Number.NaN));

function actualCounts() {
	const agents = readdirSync(join(ROOT, ".pi", "agents")).filter(
...
test("current-state docs carry no stale roster, skill, or extension counts", () => {
	...
	for (const doc of CURRENT_STATE_DOCS) {
		...
		for (const m of text.matchAll(phrase)) {
			...
			const nounToken = m[2];
			if (countToken === undefined || nounToken === undefined) continue;
			const n = toNumber(countToken);
			const noun = nounToken.toLowerCase().replace(/s?$/, "s") as keyof typeof actual;
			const expected = actual[noun];
			if (expected === undefined || Number.isNaN(n)) continue;
